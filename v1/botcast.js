...
function isAdmin(message) {
	return message.member.hasPermission('ADMINISTRATOR') || message.author.id == config.global_admin
}
		...
		case "help":
			...
			if (isAdmin(message)) {
				const embed = {
					...
					"description": "Pour executer ces commandes, utilisez `@botcast` suivit de la commande et des arguments! Vous pouvez aussi préfixer votre commande par `?`.\n__**Exemple:**__ `@botcast list` ou `?list`\nLes arguments entre [] sont obligatoires, ceux entre () sont optionnels.",
					"color": 16098851,
					"fields": [
						{
							"name": "here `(numéro)`",
